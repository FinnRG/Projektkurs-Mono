...
const TagView = () => {
    ...
    useEffect(() => {
        const getTag = () => {
            client.get('/tag/get', { params: { tag_id: params.tag_id } })
                .then((resp) => setTag(resp.data));
        }

        const getVideos = () => {
            client.get('/tag/videos', { params: { tag_id: params.tag_id } })
    ...
    return <>
        ...
        {videos.length > 0 && (<p className='title is-size-5'>Videos with this tag:</p>)}
