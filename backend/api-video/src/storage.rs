...
impl Store {
    pub async fn new() -> Self {
        let session = SessionBuilder::new().known_node(SCYLLA_URL.as_str()).build().await.unwrap();
        session.query("CREATE KEYSPACE IF NOT EXISTS ks WITH REPLICATION = {'class' : 'SimpleStrategy', 'replication_factor' : 1}", &[]).await;

        session.query("CREATE TABLE IF NOT EXISTS ks.videos (id uuid primary key, title text, description text, author uuid, date timestamp, visibility int, status int)", &[]).await;
